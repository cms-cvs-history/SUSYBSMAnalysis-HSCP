...
void stAllInfoSave(stAllInfo info, string path){
...
void makeDataCard(string outpath, string rootPath, string ChannelName, string SignalName, double Obs, double Pred, double Sign){
   ...
   fprintf(pFile, "-------------------------------\n");
   fprintf(pFile, "bin      1 1\n");
   fprintf(pFile, "process  %s pred\n",SignalName.c_str());
   fprintf(pFile, "process  0 1\n");
   fprintf(pFile, "rate    %f %f\n",Sign,Pred);
   fprintf(pFile, "-------------------------------\n");
   fprintf(pFile, "Lumi     lnN     1.022 1.0\n");
   fprintf(pFile, "stat     shapeN2 0.333 0.333\n");
   fprintf(pFile, "%4s      shapeN2 1     -\n","mom");
   fprintf(pFile, "%4s      shapeN2 1     -\n","ias");
   fprintf(pFile, "%4s      shapeN2 1     -\n","ih");
   fprintf(pFile, "%4s      shapeN2 1     -\n","tof");
   fprintf(pFile, "%4s      shapeN2 1     -\n","pu");
...
void saveHistoForLimit(TH1* histo, string Name){      
      TH1* statup   = (TH1*)histo->Clone((Name+"_statUp").c_str());
      TH1* statdown = (TH1*)histo->Clone((Name+"_statDown").c_str());
      if(Name=="pred"){
         for(int ibin=1; ibin<=statup->GetXaxis()->GetNbins(); ibin++){
            statup  ->SetBinContent(ibin,1.1*statup  ->GetBinContent(ibin) + 3*histo->GetBinError(ibin));
            statdown->SetBinContent(ibin,0.9*statdown->GetBinContent(ibin) - 3*histo->GetBinError(ibin));
         }
      }else{
         for(int ibin=1; ibin<=statup->GetXaxis()->GetNbins(); ibin++){
            statup  ->SetBinContent(ibin,statup  ->GetBinContent(ibin) + 3*histo->GetBinError(ibin));
            statdown->SetBinContent(ibin,statdown->GetBinContent(ibin) - 3*histo->GetBinError(ibin));
         }
      }
      histo   ->Write( Name             .c_str());
      statup  ->Write((Name+"_statUp"  ).c_str());
      statdown->Write((Name+"_statDown").c_str());
}
...
void saveVariationHistoForLimit(TH1* histo, TH1* vardown, string Name, string variationName){
      ...
      //TH1* vardown = (TH1*)histo ->Clone((Name+"_"+variationName+"Down").c_str());
...
/*
   ...
   TH2D*  MassPred      = (TH2D*)GetObjectFromPath(InputFileData, "Pred_Mass");

   TH2D*  MassSign   = (TH2D*)GetObjectFromPath(InputFileSign, signals[s].Name+"/Mass");
   TH2D*  MassSignP  = (TH2D*)GetObjectFromPath(InputFileSign, signals[s].Name+"/Mass_SystP");
   TH2D*  MassSignI  = (TH2D*)GetObjectFromPath(InputFileSign, signals[s].Name+"/Mass_SystI");
   TH2D*  MassSignM  = (TH2D*)GetObjectFromPath(InputFileSign, signals[s].Name+"/Mass_SystM");
   TH2D*  MassSignT  = (TH2D*)GetObjectFromPath(InputFileSign, signals[s].Name+"/Mass_SystT");
   TH2D*  MassSignPU = (TH2D*)GetObjectFromPath(InputFileSign, signals[s].Name+"/Mass_SystPU");
   ...
   //build subdirectory
   int CutIndex =(int)result.Index; 
   char CutIndexStr[255];sprintf(CutIndexStr, "CutIndex%03i",CutIndex);
   out->mkdir(CutIndexStr);
   out->cd(CutIndexStr);

   //save histograms and variations
   ...
   saveHistoForLimit(MassPredProj, "pred");

   TH1D* MassSignProj   = MassSign  ->ProjectionY("MassSignProj"  ,CutIndex+1,CutIndex+1);
   TH1D* MassSignProjP  = MassSignP ->ProjectionY("MassSignProjP" ,CutIndex+1,CutIndex+1);
   TH1D* MassSignProjI  = MassSignI ->ProjectionY("MassSignProjI" ,CutIndex+1,CutIndex+1);
   TH1D* MassSignProjM  = MassSignM ->ProjectionY("MassSignProjM" ,CutIndex+1,CutIndex+1);
   TH1D* MassSignProjT  = MassSignT ->ProjectionY("MassSignProjT" ,CutIndex+1,CutIndex+1);
   TH1D* MassSignProjPU = MassSignPU->ProjectionY("MassSignProjPU",CutIndex+1,CutIndex+1);
   MassSignProjPU->Scale(MassSignProj->Integral()/MassSignProjPU->Integral());

   saveHistoForLimit(MassSignProj, modelName);
   saveVariationHistoForLimit(MassSignProj, MassSignProjP , modelName, "mom");
   saveVariationHistoForLimit(MassSignProj, MassSignProjI , modelName, "ias");
   saveVariationHistoForLimit(MassSignProj, MassSignProjM , modelName, "ih");
   saveVariationHistoForLimit(MassSignProj, MassSignProjT , modelName, "tof");
   saveVariationHistoForLimit(MassSignProj, MassSignProjPU, modelName, "pu");
   ...
   MassSignProj->GetXaxis()->SetRangeUser(0,500);
   MassSignProj->SetMarkerStyle(21);
   MassSignProj->SetMarkerColor(4);
   MassSignProj->SetMarkerSize(1.5);
   MassSignProj->SetLineColor(4);
   MassSignProj->SetFillColor(4);
   ...
   TLegend* leg = new TLegend(0.65,0.70,1.00,0.95);
   leg->SetHeader(NULL);
   leg->SetFillColor(0);
   leg->SetBorderSize(0);
   leg->AddEntry(MassSignProj,modelName.c_str(), "F");
...
//   TH1* MassSignProjRatio  = (TH1D*)MassSignProj ->Clone(modelName.c_str() ); MassSignProjRatio ->Divide(MassSignProj);  MassSignProjRatio ->SetLineColor(1); MassSignProjRatio ->SetMarkerColor(1); MassSignProjRatio ->SetMarkerStyle(0);
//   TH1* MassSignProjPRatio = (TH1D*)MassSignProjP->Clone("mom");              MassSignProjPRatio->Divide(MassSignProj);  MassSignProjPRatio->SetLineColor(2); MassSignProjPRatio->SetMarkerColor(2); MassSignProjPRatio->SetMarkerStyle(20);
//   TH1* MassSignProjIRatio = (TH1D*)MassSignProjI->Clone("Ias");              MassSignProjIRatio->Divide(MassSignProj);  MassSignProjIRatio->SetLineColor(4); MassSignProjIRatio->SetMarkerColor(4); MassSignProjIRatio->SetMarkerStyle(21);
//   TH1* MassSignProjMRatio = (TH1D*)MassSignProjM->Clone("Ih");               MassSignProjMRatio->Divide(MassSignProj);  MassSignProjMRatio->SetLineColor(3); MassSignProjMRatio->SetMarkerColor(3); MassSignProjMRatio->SetMarkerStyle(22);
//   TH1* MassSignProjTRatio = (TH1D*)MassSignProjT->Clone("TOF");              MassSignProjTRatio->Divide(MassSignProj);  MassSignProjTRatio->SetLineColor(8); MassSignProjTRatio->SetMarkerColor(8); MassSignProjTRatio->SetMarkerStyle(23);
//   TH1* MassSignProjLRatio = (TH1D*)MassSignProjPU->Clone("pu");              MassSignProjLRatio->Divide(MassSignProj);  MassSignProjLRatio->SetLineColor(6); MassSignProjLRatio->SetMarkerColor(6); MassSignProjLRatio->SetMarkerStyle(33);

   TH1* MassSignProjRatio  = (TH1D*)MassSignProj ->Clone(modelName.c_str() );  MassSignProjRatio ->SetLineColor(1); MassSignProjRatio ->SetMarkerColor(1); MassSignProjRatio ->SetMarkerStyle(0);
   ...
   MassSignProjRatio->GetXaxis()->SetRangeUser(0,500);
   ...
   TLegend* leg2 = new TLegend(0.65,0.70,1.00,0.95);
   leg2->SetHeader("Variations");
   leg2->SetFillColor(0);
   leg2->SetBorderSize(0);
   leg2->AddEntry(MassSignProjRatio,modelName.c_str(), "L");
   ...
   SaveCanvas(c2, InputPattern+"/SHAPEEXCLUSION/shape_"+modelName, "plot", false);
   ...
   //Extract the limit
   TString inFileName = datacardPath + "_Hybrid_m2lnQ.root";
   FILE* pFileTmp = fopen(inFileName.Data(),"r");
   if(pFileTmp){
        fclose(pFileTmp);
        TString plotName = "/tmp/LANDSPLOT";
	result.XSec_Exp2Down = signals[s].XSec * extractLimitAtQuantile(inFileName, plotName+"_-2sigma", 0.0275 );
	result.XSec_ExpDown  = signals[s].XSec * extractLimitAtQuantile(inFileName, plotName+"_-1sigma", 0.16 );
	result.XSec_Exp      = signals[s].XSec * extractLimitAtQuantile(inFileName, plotName+"_median", 0.5 );
	result.XSec_ExpUp    = signals[s].XSec * extractLimitAtQuantile(inFileName, plotName+"_1sigma", 0.84 );
	result.XSec_Exp2Up   = signals[s].XSec * extractLimitAtQuantile(inFileName, plotName+"_2sigma", 0.975 );
	result.XSec_Obs      = signals[s].XSec * extractLimitAtQuantile(inFileName, plotName+"_observed", -1 );

        printf("OBS LIMITS = %6.2E\n",result.XSec_Obs);
        printf("EXP LIMITS = %6.2E %6.2E |%6.2E| %6.2E %6.2E\n",result.XSec_Exp2Down,result.XSec_ExpDown,result.XSec_Exp,result.XSec_ExpUp,result.XSec_Exp2Up);
   }
   //remove all temporary landS histograms
   system((string("rm ") + datacardPath + "*.root" ).c_str());

   stAllInfoSave(result, InputPattern+"/SHAPEEXCLUSION/"+modelName+".txt");
...
*/
   ...
   string outpath = string("Results/SHAPEEXCLUSION/");
...
//   TGraph* Tk_Obs_GluinoF0  = MakePlot(pFile,talkFile,TkPattern,syst,"Gluino  (f=00\\%)", 2, "Gluino300_f0" , "Gluino400_f0" , "Gluino500_f0" , "Gluino600_f0" , "Gluino700_f0", "Gluino800_f0", "Gluino900_f0", "Gluino1000_f0" );
   TGraph* Tk_Obs_GluinoF1  = MakePlot(pFile,talkFile,TkPattern,syst,"Gluino (f=10\\%)", 1, "Gluino300_f1" , "Gluino400_f1" , "Gluino500_f1" , "Gluino600_f1" , "Gluino700_f1", "Gluino800_f1", "Gluino900_f1", "Gluino1000_f1", "Gluino1100_f1", "Gluino1200_f1" );
   TGraph* Tk_Obs_GluinoZF1 = MakePlot(pFile,talkFile,TkPattern,syst,"Gluino Z2 (f=10\\%)", 1, "Gluino600Z_f1" , "Gluino700Z_f1", "Gluino800Z_f1");
   TGraph* Tk_Obs_GluinoF5  = MakePlot(pFile,talkFile,TkPattern,syst,"Gluino (f=50\\%)", 1, "Gluino300_f5" , "Gluino400_f5" , "Gluino500_f5" , "Gluino600_f5" , "Gluino700_f5", "Gluino800_f5", "Gluino900_f5", "Gluino1000_f5", "Gluino1100_f5", "Gluino1200_f5" );
//   TGraph* Tk_Obs_GluinoNF0 = MakePlot(pFile,talkFile,TkPattern,syst,"GluinoN (f=00\\%)", 2, "Gluino300N_f0", "Gluino400N_f0", "Gluino500N_f0", "Gluino600N_f0", "Gluino700N_f0", "Gluino800N_f0", "Gluino900N_f0", "Gluino1000N_f0" );
   TGraph* Tk_Obs_GluinoNF1 = MakePlot(pFile,talkFile,TkPattern,syst,"GluinoN (f=10\\%)", 1, "Gluino300N_f1", "Gluino400N_f1", "Gluino500N_f1", "Gluino600N_f1", "Gluino700N_f1", "Gluino800N_f1", "Gluino900N_f1", "Gluino1000N_f1", "Gluino1100N_f1", "Gluino1200N_f1" );
...
//   TGraph* Tk_Obs_Stop2C    = MakePlot(pFile,talkFile,TkPattern,syst,"Stop    (2C)     ", 2, "Stop130_2C"   , "Stop200_2C"   , "Stop300_2C"   , "Stop400_2C"   , "Stop500_2C"   , "Stop600_2C" , "Stop700_2C" , "Stop800_2C"                    );
   TGraph* Tk_Obs_Stop      = MakePlot(pFile,talkFile,TkPattern,syst,"Stop"               , 1, "Stop130"      , "Stop200"      , "Stop300"      , "Stop400"      , "Stop500"      , "Stop600"    , "Stop700"    , "Stop800"                       );
   //TGraph* Tk_Obs_StopZ     = MakePlot(pFile,talkFile,TkPattern,syst,"Stop Z2"            , 2, "Stop300Z"     , "Stop400Z"     , "Stop500Z");
   TGraph* Tk_Obs_StopN     = MakePlot(pFile,talkFile,TkPattern,syst,"StopN"              , 1, "Stop130N"     , "Stop200N"     , "Stop300N"     , "Stop400N"     , "Stop500N"     , "Stop600N"   , "Stop700N"   , "Stop800N"                      );

   TGraph* Tk_Obs_GMStau    = MakePlot(pFile,talkFile,TkPattern,syst,"GMSB Stau"          , 1, "GMStau100"    , "GMStau126"    , "GMStau156"    , "GMStau200"    , "GMStau247"    , "GMStau308", "GMStau370", "GMStau432", "GMStau494"    );
   TGraph* Tk_Obs_PPStau    = MakePlot(pFile,talkFile,TkPattern,syst,"Pair Prod. Stau  ", 1, "PPStau100"    , "PPStau126"    , "PPStau156"    , "PPStau200"    , "PPStau247","PPStau308");

   TGraph* Tk_Obs_DCRho08HyperK    = MakePlot(pFile,talkFile,TkPattern,syst,"DiChamp    Rho08  ", 1, "DCRho08HyperK100"    , "DCRho08HyperK121"    , "DCRho08HyperK182"    , "DCRho08HyperK242"    , "DCRho08HyperK302",    "DCRho08HyperK350"    ,    "DCRho08HyperK370"    , "DCRho08HyperK390"    ,  "DCRho08HyperK395"    ,  "DCRho08HyperK400"    ,  "DCRho08HyperK410"    ,    "DCRho08HyperK420"    ,  "DCRho08HyperK500");  
   TGraph* Tk_Obs_DCRho12HyperK    = MakePlot(pFile,talkFile,TkPattern,syst,"DiChamp    Rho12  ", 1,"DCRho12HyperK100"    , "DCRho12HyperK182"    , "DCRho12HyperK302"    , "DCRho12HyperK500" , "DCRho12HyperK530"  , "DCRho12HyperK570"     ,"DCRho12HyperK590"     , "DCRho12HyperK595" , "DCRho12HyperK600"   , "DCRho12HyperK610"    ,"DCRho12HyperK620"    ,"DCRho12HyperK700");
   TGraph* Tk_Obs_DCRho16HyperK    = MakePlot(pFile,talkFile,TkPattern,syst,"DiChamp    Rho16  ", 1,"DCRho16HyperK100"    , "DCRho16HyperK182"    , "DCRho16HyperK302"    , "DCRho16HyperK500"    , "DCRho16HyperK700" , "DCRho16HyperK730" , "DCRho16HyperK770"   , "DCRho16HyperK790"     , "DCRho16HyperK795" , "DCRho16HyperK800"     ,"DCRho16HyperK820"    ,"DCRho16HyperK900");
...
void CheckSignalUncertainty(FILE* pFile, FILE* talkFile, string InputPattern){
  ...
  bool IsTkOnly = (InputPattern.find("Type0",0)<std::string::npos);
   ...
   for(unsigned int s=0;s<Models.size();s++){
        stAllInfo tmp(InputPattern+"/SHAPEEXCLUSION" + "/"+Models[s]+".txt");
...
TGraph* MakePlot(FILE* pFile, FILE* talkFile, string InputPattern, string syst, string ModelName, int XSectionType, string Mass0, string Mass1, string Mass2, string Mass3, string Mass4, string Mass5, string Mass6, string Mass7, string Mass8, string Mass9,string Mass10, string Mass11, string Mass12, string Mass13){
   ...
   if(Mass0!=""){Infos[0] = stAllInfo(InputPattern+"/SHAPEEXCLUSION" + syst + "/"+Mass0+".txt"); N=1;}
   if(Mass1!=""){Infos[1] = stAllInfo(InputPattern+"/SHAPEEXCLUSION" + syst + "/"+Mass1+".txt"); N=2;}
   if(Mass2!=""){Infos[2] = stAllInfo(InputPattern+"/SHAPEEXCLUSION" + syst + "/"+Mass2+".txt"); N=3;}
   if(Mass3!=""){Infos[3] = stAllInfo(InputPattern+"/SHAPEEXCLUSION" + syst + "/"+Mass3+".txt"); N=4;}
   if(Mass4!=""){Infos[4] = stAllInfo(InputPattern+"/SHAPEEXCLUSION" + syst + "/"+Mass4+".txt"); N=5;}
   if(Mass5!=""){Infos[5] = stAllInfo(InputPattern+"/SHAPEEXCLUSION" + syst + "/"+Mass5+".txt"); N=6;}
   if(Mass6!=""){Infos[6] = stAllInfo(InputPattern+"/SHAPEEXCLUSION" + syst + "/"+Mass6+".txt"); N=7;}
   if(Mass7!=""){Infos[7] = stAllInfo(InputPattern+"/SHAPEEXCLUSION" + syst + "/"+Mass7+".txt"); N=8;}
   if(Mass8!=""){Infos[8] = stAllInfo(InputPattern+"/SHAPEEXCLUSION" + syst + "/"+Mass8+".txt"); N=9;}
   if(Mass9!=""){Infos[9] = stAllInfo(InputPattern+"/SHAPEEXCLUSION" + syst + "/"+Mass9+".txt"); N=10;}
   if(Mass10!=""){Infos[10] = stAllInfo(InputPattern+"/SHAPEEXCLUSION" + syst + "/"+Mass10+".txt"); N=11;}
   if(Mass11!=""){Infos[11] = stAllInfo(InputPattern+"/SHAPEEXCLUSION" + syst + "/"+Mass11+".txt"); N=12;}
   if(Mass12!=""){Infos[12] = stAllInfo(InputPattern+"/SHAPEEXCLUSION" + syst + "/"+Mass12+".txt"); N=13;}
   if(Mass13!=""){Infos[13] = stAllInfo(InputPattern+"/SHAPEEXCLUSION" + syst + "/"+Mass13+".txt"); N=14;}
...
stAllInfo Exclusion(string pattern, string modelName, string signal, double Ratio_0C, double Ratio_1C, double Ratio_2C, string syst){
   ...
   string outpath = pattern + "/SHAPEEXCLUSION/";
   if(syst!=""){outpath = pattern + "/SHAPEEXCLUSION" + syst + "/";}
   MakeDirectories(outpath);
...
//    if(NPred<1E-4){continue;} //This will never be the selection which gives the best expected limit (cutting too much on signal) --> Slowdown computation for nothing...
   ...
   stAllInfoSave(toReturn, outpath+"/"+modelName+".txt");
   return toReturn;
}
...
void DrawModelLimitWithBand(string InputPattern, string inputmodel)
{
   ...
   if(IsTkOnly)   SaveCanvas(c1,"Results/SHAPEEXCLUSION/", string("Tk"+ inputmodel + "ExclusionLog"));
   else    SaveCanvas(c1,"Results/SHAPEEXCLUSION/", string("Mu"+ inputmodel + "ExclusionLog"));
   delete c1;
...
void DrawRatioBands(string InputPattern, string inputmodel)
{
   ...
   if(IsTkOnly)   SaveCanvas(c1,"Results/SHAPEEXCLUSION/", string("Tk"+ inputmodel + "LimitsRatio"));
   else    SaveCanvas(c1,"Results/SHAPEEXCLUSION/", string("Mu"+ inputmodel + "LimitsRatio"));
